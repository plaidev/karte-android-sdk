...
class AtomFeedGenerator
  ...
  def generate
    ...
    # Feed metadata
    feed.add_element('title').text = 'KARTE for App Android SDK Release Notes'
  ...
  def add_release_entry(feed)
    ...
    title = "KARTE for App Android SDK"
    ...
    # Content: All modules in single entry (using CDATA to preserve UTF-8 encoding)
    ...
    content_element = entry.add_element('content', 'type' => 'html')
    content_element.add(REXML::CData.new(content_html))
...
def main
  if ARGV.length != 3
    warn 'Usage: generate_rss_feed.rb <changelog_path> <feed_url> <link_url>'
    exit 1
  end
